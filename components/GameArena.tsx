...
export default function GameArena({ onGameOver }: GameArenaProps) {
    ...
    // --- פונקציית דיבור (באנגלית - מקריא את התעתיק) ---
    const speakText = (text: string) => {
        ...
        // 2. הגדרה לאנגלית (עובד ב-100% מהמכשירים)
        utterance.lang = "en-US";
        utterance.rate = 0.9; // קצת איטי יותר כדי שיהיה ברור

        // 3. תיקון לבאג של כרום (Garbage Collection)
        // @ts-ignore
    ...
    const finishGame = (finalScoreValue: number, finalLivesValue: number) => {
        ...
        const completedAll =
            currentIndex >= questions.length - 1 && finalLivesValue > 0;
    ...
    const getButtonStyle = (option: string) => {
        ...
        const baseClass =
            "w-full py-4 text-xl font-bold border-2 rounded-xl transition-all shadow-sm ";

        if (feedback === null) {
            return (
                baseClass +
                "bg-white border-gray-200 text-gray-700 active:bg-gray-100 hover:border-blue-300"
            );
        }
    ...
    if (isLoading)
        return <div className="text-center p-10">מכין את הזירה...</div>;
    ...
    return (
        <div className="relative flex flex-col h-full bg-gray-50 overflow-hidden">
            ...
            <div className="relative flex justify-between items-center p-4 bg-white shadow-sm z-10 h-16">
                ...
                {/* טיימר */}
                <div
                    className={`font-mono font-bold text-xl z-20 relative ${timeLeft <= 3
                            ? "text-[#DA0000] animate-pulse"
                            : "text-gray-700"
                        }`}
                >
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-6 text-center space-y-6">
                ...
                {/* כפתור הדיבור - עכשיו מעביר את התעתיק באנגלית */}
                <button
                    onClick={() => speakText(currentQ.transliteration)}
                ...
                <div className="h-8">
                    {feedback === "TIMEOUT" && (
                        <span className="text-[#DA0000] font-bold animate-pulse">
                            נגמר הזמן!
                        </span>
                    )}
                </div>
